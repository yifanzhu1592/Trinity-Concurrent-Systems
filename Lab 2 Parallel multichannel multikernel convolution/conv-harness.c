/*Assignment by Utkarsh Gupta & Yifan Zhu*/
      ...
      for ( h = 0; h < height; h++ ) {
           ...
           double sum = 0;
              ...
              for ( x = 0; x < kernel_order; x++) {
                ...
                //use 4 c values at once 
                  for ( c = 0; c < nchannels; c=c+4 ) {      
                        
                       //load value of kernels
                       const float kernel0 = (float)kernels[m][c][x][y];
                       const float kernel1 = (float)kernels[m][c+1][x][y];
                       const float kernel2 = (float)kernels[m][c+2][x][y];
                       const float kernel3 = (float)kernels[m][c+3][x][y];
                       
                       
                       //store kernels in an array so that we can access it bu m128
                       const float kernellArray[4]={kernel0,kernel1,kernel2,kernel3};
                       
                       //loading 4 values of images and kernels for c loop
                      __m128 images = _mm_loadu_ps(&(image[w + x][h + y][c]));
                      __m128 kernelss = _mm_loadu_ps(&(kernellArray[0]));
                      //image[w + x][h + y][c] * kernels[m][c][x][y];
                      __m128 product = _mm_mul_ps(images, kernelss);     
                      //convert floats to double
                      __m128d prod =_mm_cvtps_pd(product);
                      //shuffle to get the other two floats to top
                      product = _mm_shuffle_ps(product, product, _MM_SHUFFLE(1,0,3,2));
                      //convert the last two floats to double
                      __m128d sum16= _mm_cvtps_pd(product);
                      //add the four values
                      __m128d fsum= _mm_hadd_pd(prod, sum16);
                      //add the current sum to the total sum
                      fsum= _mm_hadd_pd(fsum, fsum);
                      //get the sum into a double type
                      sum=sum + fsum[0];
                }
             }
          }
